...
var $widths;
	...
	function Titre($titre, $y){
		...
		$this->MultiCell(70,6,utf8_decode($this->CodeBreaks($titre)),0,'R');
	...
	function Competences($competences){
			...
			$this->SetTextColor(0,0,255);
// 			$this->ln(3);
			$this->Cell(0,8,utf8_decode($competences->attributes->getNamedItem("nom")->value),0,1,'L');
			foreach($competences->childNodes as $competence){
				if($competence->nodeType==1){
					$this->SetFont('gillb','',14);
					$this->SetTextColor(0,0,255);
					$this->Cell(0,8,utf8_decode($competence->attributes->getNamedItem('nom')->value),0,1,'C');
					$this->SetTextColor(0,0,0);
	...
	function TraitementPdf($cv){
		foreach($cv->titre as $titre){
			...
			$this->SetTextColor(0,0,255);
			$this->ln(3);
			$this->Cell(0,8,utf8_decode($titre["nom"]),0,1,'L');
			$yend;
			$xorig = $this->getX();
			$count = 0;
